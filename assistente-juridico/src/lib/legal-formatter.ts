...
export function plainTextToLegalHtml(text: string): string {
  ...
  while (i < lines.length) {
    ...
    const isAllCaps = line === line.toUpperCase() && line.length > 2 && line.length < 120;
    ...
    if (isTitle || (isAllCaps && !isCitation)) {
      const align = isCentered ? 'text-align: center' : 'text-align: center';
      htmlParts.push(
        `<p style="${align}; font-weight: bold; text-transform: uppercase; text-indent: 0; margin: 0.4cm 0 0.2cm;">${escapedLine}</p>`
      );
    } else if (isCitation) {
      htmlParts.push(
    ...
    } else if (line.startsWith("Nestes termos") || line.startsWith("Termos em que") || line.startsWith("Pede deferimento") || line.startsWith("Aguarda-se")) {
      htmlParts.push(
        `<p style="text-align: left; text-indent: 0; margin-top: 0.5cm;">${escapedLine}</p>`
      );
    } else if (line.match(/^[A-Z][a-zA-ZÀ-ú\s,]+,\s+\d+\s+de\s+\w+\s+de\s+\d{4}/)) {
      htmlParts.push(
